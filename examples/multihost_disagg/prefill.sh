...
# Prefill server (KV producer) for tpu_raiden_connector MULTI-HOST disaggregated
# serving. Runs on the prefill VM; the decode VM pulls KV from here over the
# network. Raiden-only: no connector switching, no other-connector flags.
set -u
