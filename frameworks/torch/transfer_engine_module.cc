...
static nb::list HostSpanMemoryViews(
    const std::vector<kv_cache::KVCacheHostSpan>& host_spans) {
  nb::list views;
...
NB_MODULE(_transfer_engine, m) {
  ...
  nb::class_<TransferEngine>(m, "TransferEngine")
      ...
      .def("register_kv_cache", &TransferEngine::RegisterKvCache,
           nb::arg("kv_caches"))
      .def(
          "register_host_buffers",
          [](TransferEngine& self, nb::object /*host_pool*/, int64_t tp_rank) {
            self.RegisterHostBuffers(tp_rank);
          },
